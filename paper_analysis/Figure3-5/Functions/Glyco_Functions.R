### Functions script for Scanning SWATH glyco analysis
...
plot.b2b.fromfolder <- function(peak,
  ...
  cid.hcd.match <- cid.hcd %>%
    ...
    ungroup() %>%
    filter(PPM.Error < 50) %>% ### 50ppm tolerance for matching
  ...
  hcd.plot <- hcd.all.matches %>%
    ...
    geom_col(size = 0.3) +
    geom_col(data = hcd.all.matches %>% filter(Colour == "dark green"),
             colour = "dark green",
             size = 0.3, 
             width = 0.3) + 
    theme_linedraw() +
    ...
    facet_wrap(~""
                 #paste(pull(temp.glycan.info[1,"Genes"]), ## Gene
    #                  ", ",
...
### B2B plotting (matched fragments only)
...
#### make plot with all fragments,coloured by matched / oxonium to display quality of match
...
### if phrasees to add annotations
    if(hcd_annot == T){
...
### Plot protein abudnances for HP glycofrms - essentially y axis with 1dp
### Plot protein abundances from protein.normalised dataset

protein.boxplot.glycoforms <- function(gene.name, 
                            plot.title,
                            bottom.plot = F,
                            strip.size = 10.5,
                            y.axis.breaks = 0.5){
...
#### glycopeptide boxplot - also from protein.normalised
...
gp.boxplot <- function(peak.number,
  ...
  temp.plot <- temp1 %>%
    ggplot(aes(x = factor(F.Severity, levels=c("Healthy","Mild","Moderate", "Severe")),
               y = log2(Glycopeptide.Intensity), ### Glycopeptide feature intensity
               fill = F.Severity)) + 
...
##### protein normalsied gp intensity
#### glycopeptide boxplot - also from protein.normalised
...
### gp plotting for multiple glycosites
gp.boxplot.glycosites <- function(peak.number,
                                  end.plot = F,
                                  end.facet.name = NA,
                                  strip.size = 10.5,
                                  y.axis.breaks = 0.5){
  ...
  temp1 <- filter(protein.normalised, peak_num == peak.number) %>%
    distinct() %>%
    left_join(select(dda.valid, peak_num, Glycan.Short) %>% 
                distinct()) %>%
    mutate(EndPosition = StartPosition + nchar(AA.Only) - 1,
           facet = paste(#sex, 
             #Genes, 
             #", AA",
             #StartPosition,
  ...
  temp.plot <- temp1 %>%
    ggplot(aes(x = factor(F.Severity, levels=c("Healthy","Mild", "Moderate", "Severe")),
               y = log2(gp.intensity),### raw glycan feature intensity
               fill = F.Severity)) + 
    geom_boxplot(inherit.aes = T, 
                 fill = NA, 
                 alpha=0.75, 
    ...
    theme(strip.background = element_blank(),
          ...
          plot.margin = unit(c(0, 0, 0, 0), "cm"),
          legend.position = "none") +
    scale_y_continuous(breaks = seq(-10,10,by=y.axis.breaks),
                       limits = c(log2(max(temp1$gp.intensity)) - 8, log2(max(temp1$gp.intensity)))) + 
    ylab("") +
    xlab("") + 
    scale_colour_manual(values = met.brewer("Egypt",4)) + 
    scale_fill_manual(values = met.brewer("Egypt",4)) 
  ...
  if(end.plot == T){ ### facet grid including gene name on right hand side
    ...
    temp.plot <- temp.plot +
      facet_grid(paste(end.facet.name,sep="")~facet,
                 scales = "free_y") 
...
## 
gp.boxplot.glycoforms <- function(peak.number,
                                  end.plot = F,
                                  bottom.plot = F,
                                  end.facet.name = NA,
                                  strip.size = 10.5,
  ...
  if(end.plot == T & bottom.plot == F){ ### facet grid including gene name on right hand side
  ...
  if(end.plot == F & bottom.plot == F){ ### just top labelling facet wrap
  ...
  if(end.plot == T & bottom.plot == T){ ### no top labelling but with right hand
    
    temp.plot <- temp.plot +
      facet_wrap(~paste(end.facet.name,sep=""),
                 scales = "free_y", 
                 nrow = 1,
                 strip.position = "right")
  }
  if(end.plot == F & bottom.plot == T){ ### no facet labelling at all
...
plot.xic <- function(peak,
  ...
  xic.plot <- xic %>%
    ...
    #xlim(ref.rt - 0.3, ref.rt + 0.3) +
    scale_y_continuous(expand = expansion(mult = c(0.0, 0.05)),
                       limits = c(0,max(xic$Intensity))
    ) +
    facet_wrap(~"") + 
    xlab("Retention Time") + 
    ...
    scale_x_continuous(expand = expansion(mult = c(0.0, 0.0)),
                       limits = c(ref.rt - 0.2, ref.rt + 0.2)) +
    scale_fill_manual(values = pnw_palette("Sunset2", n = 7),
                      guide = guide_legend(override.aes = list(shape = 15))) +
    scale_colour_manual(values = pnw_palette("Sunset2", n = 7),
                        guide = guide_legend(override.aes = list(shape = 15))) 
...
plot.xic.2d <- function(PlasmaPooledRep = 2,
                        peak,
                        remove_ions = c(),
                        plot_exclusion = T){
  ...
  #max.rt <- xic %>%
  #  filter(RT > ref.rt - 0.35,  
  #         RT < ref.rt + 0.35) %>%
  #  select(RT.Normalised, Intensity) %>%
  ...
  if(plot_exclusion == T){
    ...
    xic.plot <- xic.plot + 
      geom_ellipse(data = data.frame(),
                   ...
                   aes(x0 = max.rt,
                       y0 = ref.mz,
                       a = 3*3.5*0.025, b = 2*11, ### exclusion parameters
                       angle = 0),
                   colour = "#c36377",
                   fill = NA,
                   alpha = 0.4, 
...
plot.q1.ms1 <- function(peak){
  ...
  ggdraw() + 
    draw_plot(main.plot) + 
    draw_plot(inset, x = 0.45, y = 0.35, height = 0.5, width = 0.35)
...
dda.peak.extract <- function(peak_number,
  ...
  temp.table <- byonic %>%
    ...
    select(Peptide, Glycan, `M+H`, `Calc M+H`, StartPosition, ## Structure info
